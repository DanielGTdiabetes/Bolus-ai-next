...
|---|---|---|---|---|---|
| R-001 | Duplicar fórmulas entre Android e iOS | Seguridad/consistencia | Un solo módulo KMP en `shared/bolus-engine`; no existe motor Swift separado | JVM automática; iOS explícito antes de declarar compatibilidad, según ADR 0004 | Mitigación estructural aceptada; verificación iOS por commit retirada |
...
| R-003 | Aceptar un cambio que solo funciona en Windows | Consistencia | Verificación JVM/Android automática; conservar pruebas KMP/Swift para Mac autorizado | Workflow `verify.yml` + ADR 0004 | Riesgo aceptado por coste durante prioridad Android; no afirmar iOS actualizado sin prueba explícita |
...
| R-010 | Aceptar un broadcast G7 basándose solo en extras controlables | Seguridad | El puerto Android v1 no tiene rama de lectura disponible; demostrar contrato y autenticidad antes de añadirla | LC-003 + tests del puerto; faltan tests Android de emisor/permiso | Parcial: fail-closed integrado; investigación técnica pendiente |
